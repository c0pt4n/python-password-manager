...
class PasswordManagerApp(CTk):
    def __init__(self):
        ...
        self.geometry(f"400x550+{(self.winfo_screenwidth() // 2) - 200}+{(self.winfo_screenheight() // 2) - 275}")
    ...
    # Letter selection
        ...
        CTkSlider(self, from_=3, to=10, variable=self.no_of_letters,
                command=lambda v: self.update_slider_value(self.letter_slider_value, v)).pack(pady=5)
    ...
    # Digit selection
        ...
        CTkSlider(self, from_=3, to=10, variable=self.no_of_digits,
                command=lambda v: self.update_slider_value(self.digit_slider_value, v)).pack(pady=5)
    ...
    # Symbol selection
        ...
        CTkSlider(self, from_=3, to=10, variable=self.no_of_symbols,
    ...
    # Display Password
        self.password_display = CTkEntry(self, textvariable=self.generated_password, state="readonly")
        self.password_display.pack(pady=10)

    # Copy Button
        self.copy_btn = CTkButton(self, text="Copy Password", command=self.copy_password)
    ...
    def initialize_master_password(self):
        ...
        def display_recovery_key(recovery_key):
            ...
            CTkLabel(recovery_window, text="Your Recovery Key:", font=("Arial", 14)).pack(pady=10)
        ...
        # Copy button
            ...
            copy_button = CTkButton(recovery_window, text="Copy", command=copy_to_clipboard)
            ...
            done_button = CTkButton(recovery_window, text="Done", command=finish_and_login)
    ...
    # Create setup window
        ...
        CTkLabel(setup_window, text="Create Master Password", font=("Arial", 14)).pack(pady=10)
    ...
    def reset_master_password(self):
        ...
        # Verify recovery key
            self.cursor.execute("SELECT * FROM masterpassword WHERE recoverykey=?", (hashed_key,))
    ...
    # Create reset window
        ...
        CTkLabel(reset_window, text="Enter Recovery Key", font=("Arial", 14)).pack(pady=10)
    ...
    def open_vault(self):
        ...
        # Verify master password from the database
            self.cursor.execute("SELECT * FROM masterpassword WHERE id=1 AND password=?", (hashed_password,))
            result = self.cursor.fetchone()
    ...
    # Create password entry window
        ...
        CTkLabel(vault_window, text="Enter Master Password", font=("Arial", 14)).pack(pady=10)

        password_entry = CTkEntry(vault_window, show="*", width=200)
        password_entry.pack(pady=10)
        password_entry.focus()

        unlock_btn = CTkButton(vault_window, text="Unlock", command=check_master_password)
        ...
        reset_btn = CTkButton(vault_window, text="Forgot Password?", command=reset_master_password_with_close)
...
class VaultWindow(CTkToplevel):
    ...
    def create_vault_ui(self):
        ...
        # Scrollbar
        scrollbar = ttk.Scrollbar(self, orient="vertical", command=self.vault_table.yview)
    ...
    def load_entries(self):
        ...
        if entries:
            for entry in entries:
                ...
                masked_password = '*' * len(decrypted_password)  # Replace password with asterisks
                self.vault_table.insert("", "end", values=(entry[1], entry[2], masked_password), iid=entry[0])
        else:
    ...
    def update_entry(self):
        ...
        if selected_item:
            ...
            # Get updated values
            new_account = simpledialog.askstring("Input", "Update Account Name:", initialvalue=current_account)
            new_username = simpledialog.askstring("Input", "Update Username:", initialvalue=current_username)
            new_password = simpledialog.askstring("Input", "Update Password:", initialvalue=current_password)
